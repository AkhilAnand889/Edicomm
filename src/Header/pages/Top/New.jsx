import React,{useState} from 'react'
import {Button,Card,Form,Collapse,FormControl} from 'react-bootstrap'
import { Link } from 'react-router-dom';

function New() {
    const [isPaused, setIsPaused] = useState(false);
    const [open, setOpen] = useState(false);
    const handleSubmit = ()=>{
        console.log('hello akhil its not working')
      }
  
  return (
    <div>
      <div className='kui1'>
         <div className='dir'>
          <div className='dir2'>
            <h5 className='head9'> <Link to={'/new'}  style={{ color: 'white' }}>New</Link></h5>
            <h5 className='head10'><Link to={'/backgrounds'}  style={{ color: 'white' }}>Backgrounds</Link></h5>
            <h5 className='head11'><Link to={'/valentines'}  style={{ color: 'white' }}>Valentines</Link></h5>
            <h5 className='head4'><Link to={'/lunar'}  style={{ color: 'white' }}>Lunar</Link></h5>
            <h5 className='head5'><Link to={'/vertical'}  style={{ color: 'white' }}>Vertical</Link></h5>
             </div>
          </div>
          <div>
            <img className='good' src="http://img.gifmagazine.net/gifmagazine/images/704479/original.gif" alt="" />         
            <h4 className='middle'>Astounding Royalty Free Stock Music</h4>
          <h3 className='last'>For you to use in your next video project, for free!</h3>
            <div className='search'>
        <Form onSubmit={handleSubmit} inline='true'>
            <FormControl   type="text" placeholder="Search Music" className="mrm-sm-2" />
            <Button className='ravan' variant="outline-success">Search</Button>
        </Form>
        </div>    
          </div>
          <div className='ash'>
        <img className='short' src="data:image/jpeg;base64," alt="" />
        <h4 className='crack'>Crackzon</h4>
        <h4 className='tech'>Technology</h4>
        <h5 className='y1'>Find what you need on Crackzon Technology</h5>
        <p className='q1'>Discover millions of stock videos & templates, royalty-free music tracks, photos & more. <br /> All with unlimited downloads &  simple commercial licensing for any project.</p>
        <Button className='button1' >Upload Here</Button>{' '}
        </div>
   
   
        <div>
          <div className='lar'>
        <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://coolhdwall.com/storage/202103/alone-boy-moon-night-scenery-art-2k-wallpaper-2560x1440.jpg" alt=""  diabled/>
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://c4.wallpaperflare.com/wallpaper/264/666/478/3-316-16-9-aspect-ratio-s-sfw-wallpaper-preview.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      <br />
      <div>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://c4.wallpaperflare.com/wallpaper/237/293/295/3-316-16-9-aspect-ratio-s-sfw-wallpaper-preview.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <div>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://i.pinimg.com/originals/39/e6/ec/39e6ec0ceba16fe3fb063328df0413a3.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <div className='u1'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://c4.wallpaperflare.com/wallpaper/900/92/1002/3-316-16-9-aspect-ratio-s-sfw-wallpaper-preview.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <div className='y10'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://wallpaperaccess.com/full/4311000.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <div className='y9'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://media.istockphoto.com/id/1428467670/photo/synthwave-retrowave-styled-background.jpg?b=1&s=170667a&w=0&k=20&c=VJ8nP4HlmdVJYvsxTXobvDT1Pb8MmrnhO_D3eCb3JbQ=" alt="" />
        <Card.Body xs={2}>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body >
      </Card>
      </div>
      <br />
      <div className='tr'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://wallpapercave.com/wp/wp8279794.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <div className='hs'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://c4.wallpaperflare.com/wallpaper/31/105/276/retrowave-synthwave-neon-ultrawide-wallpaper-thumb.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <br />
      <div className='hf'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://c4.wallpaperflare.com/wallpaper/562/787/767/synthwave-vaporwave-planet-screen-shot-music-hd-wallpaper-preview.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>      
      <br />
      <div className='ht'>
      <Card xs={1} md={2} className="g-4">
       <img className='ima3' src="https://wallpaper.dog/large/20441473.jpg" alt="" />
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
      <div className='h18'>
      <Card xs={1} md={2} className="g-4">
      <Link to='first'><img className='ima3' src="https://www.pixelstalk.net/wp-content/uploads/images6/Retrowave-Wallpaper-Desktop.jpg" onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}  style={{
            animationPlayState: isPaused ? 'paused' : 'running',}} alt="" /></Link>
        <Card.Body>
        <button className='play'>Download</button> 
        <button className='plays'>Play</button> 
          <Card.Text>
            <h3>Star Boy</h3>
            <span>by  : The Weekend</span>
          </Card.Text>
        </Card.Body>
      </Card>
      </div>
        </div>
        <br />
        <div className='but7'>
        <Button
        onClick={() => setOpen(!open)}
        aria-controls="example-collapse-text"
        aria-expanded={open} className='but8'
      >
        EDICOMM
      </Button>
      <div style={{ minHeight: '150px' }}>
        <Collapse in={open} dimension="width">
          <div id="example-collapse-text">
            <Card body style={{ width: '400px' }}>
            EdiComm is a free gallery of awesome <br /> stock video clips, music tracks, <br /> sound effects and video templates
            </Card>
          </div>
        </Collapse>
      </div>
      </div>
       </div>
    </div>
  )
}

export default New
